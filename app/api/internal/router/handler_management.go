	...
	if err != nil {
		...
		if errors.As(err, &vErr) {
			...
			for _, e := range vErr.Errors() {
				f.Is(e, management.ErrInvalidSpotID, httputil.NewField(paramKeySpotID, "Must be a non empty string."))
			}
...
func (h *managementHandler) spots(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if errors.As(err, &vErr) {
			f := httputil.NewFields()
			for _, e := range vErr.Errors() {
				f.Is(e, management.ErrInvalidSearchQuery, httputil.NewField("query", "Must not exceed character limit."))
				f.Is(e, management.ErrInvalidCountryCode, httputil.NewField("country", "Must be a valid ISO-2 country code."))
				f.Is(e, management.ErrInvalidNorthEastLatitude, httputil.NewField("ne_lat", "Must be a valid latitude."))
				f.Is(e, management.ErrInvalidNorthEastLongitude, httputil.NewField("ne_lon", "Must be a valid longitude."))
				f.Is(e, management.ErrInvalidSouthWestLatitude, httputil.NewField("sw_lat", "Must be a valid latitude."))
				f.Is(e, management.ErrInvalidSouthWestLongitude, httputil.NewField("sw_lon", "Must be a valid longitude."))
			}
			httputil.WriteFieldErrors(w, r, f)
			return
		}
...
func (h *managementHandler) createSpot(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if errors.As(err, &vErr) {
			f := httputil.NewFields()
			for _, e := range vErr.Errors() {
				f.Is(e, management.ErrInvalidSpotName, httputil.NewField("name", "Must be a non empty string."))
				f.Is(e, management.ErrInvalidCountryCode, httputil.NewField("country_code", "Must be a valid ISO-2 country code."))
				f.Is(e, management.ErrInvalidLocality, httputil.NewField("locality", "Must be a non empty string."))
				f.Is(e, management.ErrInvalidLatitude, httputil.NewField("latitude", "Must be a valid latitude."))
				f.Is(e, management.ErrInvalidLongitude, httputil.NewField("longitude", "Must be a valid longitude."))
			}
			httputil.WriteFieldErrors(w, r, f)
			return
		}
...
func (h *managementHandler) location(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputil.WriteFieldError(w, r, httputil.NewField("lat", "Must be a valid latitude."))
		return
	}

	longitude, err := httputil.QueryParamFloat(r, "lon")
	if err != nil {
		httputil.WriteFieldError(w, r, httputil.NewField("lon", "Must be a valid longitude."))
		return
	}
	...
	if err != nil {
		...
		if errors.As(err, &vErr) {
			f := httputil.NewFields()
			for _, e := range vErr.Errors() {
				f.Is(e, management.ErrInvalidLatitude, httputil.NewField("lat", "Must be a valid latitude."))
				f.Is(e, management.ErrInvalidLongitude, httputil.NewField("lon", "Must be a valid longitude."))
			}
			httputil.WriteFieldErrors(w, r, f)
			return
		}
